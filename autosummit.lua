--[[
    ╔══════════════════════════════════════════╗
    ║     AUTO SUMMIT PRO - MONOCHROME ELITE   ║
    ║     Version 2.0 | By Script Pro          ║
    ╚══════════════════════════════════════════╝
    Features:
    - Auto Detection CP (unlimited, tampil semua)
    - Auto Summit (berurutan otomatis)
    - Manual Summit (klik 1 per 1)
    - Fly System (UP/DOWN/Speed)
    - Age Bypass (bypass map 3-day account requirement)
    - Badge/Summit detector ("Selamat kamu mendapatkan...")
    - Toggle F9, Drag, Minimizable
    - UI Monochrome Elegan & Responsif
]]
...
pcall(function()
    for _, gui in ipairs(player.PlayerGui:GetChildren()) do
        if gui.Name == "AutoSummitPRO" then gui:Destroy() end
    end
end)
...
local YELLOW  = Color3.fromRGB(220, 200, 80)
...
-- ══════════════════════════════════════
-- AGE BYPASS (bypass 3-day restriction)
-- ══════════════════════════════════════
local function ageBypasser()
    -- Override AccountAge check commonly used by obby games
    pcall(function()
        local mt = getrawmetatable and getrawmetatable(player) or nil
        if mt then
            local old = mt.__index
            setreadonly(mt, false)
            mt.__index = function(self, key)
                if key == "AccountAge" then return 9999 end
                if key == "MembershipType" then return Enum.MembershipType.Premium end
                return old(self, key)
            end
            setreadonly(mt, true)
        end
    end)
    -- Also fire common RemoteEvents used to check age
    pcall(function()
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") then
                local n = v.Name:lower()
                if n:match("age") or n:match("verify") or n:match("check") or n:match("join") or n:match("enter") then
                    pcall(function() v:FireServer(9999) end)
                    pcall(function() v:InvokeServer(9999) end)
                end
            end
        end
    end)
    -- Touch all entrance gates
    pcall(function()
        for _, v in ipairs(workspace:GetDescendants()) do
            if v:IsA("BasePart") then
                local n = v.Name:lower()
                if n:match("gate") or n:match("door") or n:match("enter") or n:match("barrier") or n:match("age") then
